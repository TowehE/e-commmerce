...
exports.signUp = async (req, res) => {
    try {
        ...
        const html = generateDynamicEmail(fullName, otp);
        sendEmail({
            email: newUser.email,
            html,
            subject
...
exports.logIn = async (req, res) => {
    try {
        ...
        if (error) {
            ...
            const user = await customerModel.findOne({ where: { email: email.toLowerCase() } });
            if (!user) {
...
exports.updateUser = async (req, res) => {
    try {
        ...
        const users = await customerModel.findByPk(id)
        ...
        const [updatedRowsCount] = await customerModel.update(userData, {
            where: { id: id }
        });
